...
public class Ramp{
	...
	public Shape[] getShapes(int tileWidth, int tileHeight, int scale){
		...
		int y = (int)pos.y;
		Shape[] shapes = null;
		InverseArc a = new InverseArc(new Vector(x + s01 + s01 / 2, y + s03 + s01 / 2), PI + PI / 2, PI + PI / 2 + PI / 4, s03, Color.WHITE);
		shapes = append(shapes,
			new Shape(new Vector[]{a.points[2], new Vector(x + s05, y + s00), new Vector(x + s05, y + s03 + s01 / 2), new Vector(a.points[2].x, y + s03 + s01 / 2)}, Color.WHITE));
