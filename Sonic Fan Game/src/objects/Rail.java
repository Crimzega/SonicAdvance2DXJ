...
public class Rail{
	...
	public Shape[] getShapes(int tileWidth, int tileHeight, int scale){
		...
		for(int i = 0; i < corners.length - 1; i++){
			...
			shapes = append(shapes, new Shape(new Vector[]{new Vector(x1 + s00, y1 + s00), new Vector(x2 + s00, y2 + s00), new Vector(x2 + cos(angle) * s01, y2 + sin(angle) * s01),
				new Vector(x1 + cos(angle) * s01, y1 + sin(angle) * s01),}, Color.WHITE));
		}
		for(int i = 1; i < corners.length - 1; i++){
			...
			if(difAngle < PI && angle1 == PI){ shapes = append(shapes, new InverseArc(new Vector(x1, y1), angle0, angle1, s03, Color.WHITE)); }
		}
		return shapes;
	}
