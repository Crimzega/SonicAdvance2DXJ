...
public class SettingsSubScene extends Scene{
	...
	public void init(){
		fadeTimer = 120;
		blinkTimer = state = 0;
		exit = settingKey = keySetReady = false;
	}
	...
	private void stateSettings(float dt){
		...
		if(!settingKey){
			...
			else if(enterKey && enterReady || aKey && aReady){ if(settingsSelection >= 3 && settingsSelection <= 11) settingKey = !(keySetReady = false); }
		}
		else{
			if(!keySetReady){
				keySetReady = !enterKey && !aKey;
			}
			else{
				...
				if(settingsSelection == 7) if(KeyListener.getNextJoystickButton() != -1) Loader.joyA = KeyListener.getNextJoystickButton();
				settingKey = false;
				if(settingsSelection == 8) if(KeyListener.getNextJoystickButton() != -1) Loader.joyB = KeyListener.getNextJoystickButton();
				settingKey = false;
				if(settingsSelection == 9) if(KeyListener.getNextJoystickButton() != -1) Loader.joyX = KeyListener.getNextJoystickButton();
				settingKey = false;
				if(settingsSelection == 10) if(KeyListener.getNextJoystickButton() != -1) Loader.joyStart = KeyListener.getNextJoystickButton();
				settingKey = false;
				if(settingsSelection == 11) if(KeyListener.getNextJoystickButton() != -1) Loader.joyBack = KeyListener.getNextJoystickButton();
				settingKey = false;
			}
			enterReady = backReady = upReady = downReady = leftReady = rightReady = aReady = false;
		}
		checkKeys();
		enterReady = !enterKey;
		backReady = !backKey;
