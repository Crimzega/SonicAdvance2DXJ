...
public class MenuScene extends Scene{
	...
	private void stateFadingOut(float dt){
		...
		fade.setColor(0, 80f / 255f, 224f / 255f, 1);
	...
	private void stateFadingTitle(float dt){
		int screenWidth = Window.getWidth();
		int screenHeight = Window.getHeight();
		fade.setColor(0, 80f / 255f, 224f / 255.0f, 1);
		fade.draw(0, 0, screenWidth, screenHeight, r);
		leftCloud.draw(0, screenHeight - leftCloud.getHeight() * Loader.scale, Loader.scale, Loader.scale, r);
		rightCloud.draw(screenWidth - rightCloud.getWidth() * Loader.scale, screenHeight - rightCloud.getHeight() * Loader.scale, Loader.scale, Loader.scale, r);
	...
	private void stateStartBlink(float dt){
		...
		title.draw(screenWidth / 2 - title.getWidth() / 2 * Loader.scale, screenHeight / 3 - title.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		if(blinkTimer % 10 < 5){
			start.draw(screenWidth / 2 - start.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - start.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		for(int f = 1; f < min(60.0f / (1.0f / dt), 5); f++){
			blinkTimer++;
			if(blinkTimer == 30){
	...
	private void stateStartFadeForwards(float dt){
		...
		singleplayerYellowSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale + newMovement * Loader.scale,
			screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		multiplayerWhiteSprite.draw(screenWidth / 2 - multiplayerWhiteSprite.getWidth() / 2 * Loader.scale + newMovement * Loader.scale,
			screenHeight / 3 * 2 - multiplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		for(int f = 1; f < min(60.0f / (1.0f / dt), 5); f++){
			oldMovement -= 10;
			newMovement -= 10;
		}
	...
	private void stateMenu1(float dt){
		...
		title.draw(screenWidth / 2 - title.getWidth() / 2 * Loader.scale, screenHeight / 3 - title.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		if(menuSelection == 0){
			singleplayerYellowSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale,
				screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			multiplayerWhiteSprite.draw(screenWidth / 2 - multiplayerWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - multiplayerWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 1){
			singleplayerWhiteSprite.draw(screenWidth / 2 - singleplayerWhiteSprite.getWidth() / 2 * Loader.scale,
				screenHeight / 3 * 2 - singleplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			multiplayerYellowSprite.draw(screenWidth / 2 - multiplayerYellowSprite.getWidth() / 2 * Loader.scale,
				screenHeight / 3 * 2 - multiplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
	...
	private void stateMenu1Blink(float dt){
		...
		rightCloud.draw(screenWidth - rightCloud.getWidth() * Loader.scale, screenHeight - rightCloud.getHeight() * Loader.scale, Loader.scale, Loader.scale, r);
		title.draw(screenWidth / 2 - title.getWidth() / 2 * Loader.scale, screenHeight / 3 - title.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		if(menuSelection == 0) multiplayerWhiteSprite.draw(screenWidth / 2 - multiplayerWhiteSprite.getWidth() / 2 * Loader.scale,
			screenHeight / 3 * 2 - multiplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		if(menuSelection == 1) singleplayerWhiteSprite.draw(screenWidth / 2 - singleplayerWhiteSprite.getWidth() / 2 * Loader.scale,
			screenHeight / 3 * 2 - singleplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		if(blinkTimer % 10 < 5){
			if(menuSelection == 0) singleplayerYellowSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale,
				screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			if(menuSelection == 1) multiplayerYellowSprite.draw(screenWidth / 2 - multiplayerYellowSprite.getWidth() / 2 * Loader.scale,
				screenHeight / 3 * 2 - multiplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		for(int f = 1; f < min(60.0f / (1.0f / dt), 5); f++){
			blinkTimer++;
	...
	private void stateMenu1FadeForwards(float dt){
		...
		if(menuSelection == 0){
			singleplayerYellowSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			multiplayerWhiteSprite.draw(screenWidth / 2 - multiplayerWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - multiplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 1){
			singleplayerWhiteSprite.draw(screenWidth / 2 - singleplayerWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - singleplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			multiplayerYellowSprite.draw(screenWidth / 2 - multiplayerYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - multiplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		gameStartYellowSprite.draw(screenWidth / 2 - gameStartYellowSprite.getWidth() / 2 * Loader.scale + newMovement * Loader.scale,
			screenHeight / 3 * 2 - gameStartYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		timeAttackWhiteSprite.draw(screenWidth / 2 - timeAttackWhiteSprite.getWidth() / 2 * Loader.scale + newMovement * Loader.scale,
			screenHeight / 3 * 2 - timeAttackWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		optionsWhiteSprite.draw(screenWidth / 2 - optionsWhiteSprite.getWidth() / 2 * Loader.scale + newMovement * Loader.scale,
			screenHeight / 3 * 2 - optionsWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		for(int f = 1; f < min(60.0f / (1.0f / dt), 5); f++){
			oldMovement -= 10;
			newMovement -= 10;
		}
	...
	private void stateMenu1FadeBackwards(float dt){
		...
		singleplayerWhiteSprite.setColor(1, 1, 1, 1.0f - getLinearFade((newMovement * 1.0f) / (start.getWidth() * 1.0f)));
		multiplayerWhiteSprite.setColor(1, 1, 1, 1.0f - getLinearFade((newMovement * 1.0f) / (start.getWidth() * 1.0f)));
		start.draw(screenWidth / 2 - start.getWidth() / 2 * Loader.scale + newMovement * Loader.scale, screenHeight / 3 * 2 - start.getHeight() / 2 * Loader.scale, Loader.scale,
			Loader.scale, r);
		if(menuSelection == 0){
			singleplayerYellowSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		...
		if(menuSelection == 1){
			singleplayerWhiteSprite.draw(screenWidth / 2 - singleplayerYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - singleplayerYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			multiplayerYellowSprite.draw(screenWidth / 2 - multiplayerWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - multiplayerWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		for(int f = 1; f < min(60.0f / (1.0f / dt), 5); f++){
			oldMovement += 10;
	...
	private void stateMenu2(float dt){
		...
		optionsWhiteSprite.setColor(1, 1, 1, 1);
		if(menuSelection == 0){
			gameStartYellowSprite.draw(screenWidth / 2 - gameStartYellowSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - gameStartYellowSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
			timeAttackWhiteSprite.draw(screenWidth / 2 - timeAttackWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - timeAttackWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
			optionsWhiteSprite.draw(screenWidth / 2 - optionsWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - optionsWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 1){
			gameStartWhiteSprite.draw(screenWidth / 2 - gameStartWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - gameStartWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
			timeAttackYellowSprite.draw(screenWidth / 2 - timeAttackYellowSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - timeAttackYellowSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
			optionsWhiteSprite.draw(screenWidth / 2 - optionsWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - optionsWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 2){
			...
			timeAttackWhiteSprite.draw(screenWidth / 2 - timeAttackWhiteSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - timeAttackWhiteSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
			optionsYellowSprite.draw(screenWidth / 2 - optionsYellowSprite.getWidth() / 2 * Loader.scale, screenHeight / 3 * 2 - optionsYellowSprite.getHeight() / 2 * Loader.scale,
				Loader.scale, Loader.scale, r);
		}
		if(enterKey && enterReady){
			if(menuSelection == 0){
	...
	private void stateMenu2FadeForwards(float dt){
		...
		if(menuSelection == 0){
			gameStartYellowSprite.draw(screenWidth / 2 - gameStartYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - gameStartYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			timeAttackWhiteSprite.draw(screenWidth / 2 - timeAttackWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - timeAttackWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			optionsWhiteSprite.draw(screenWidth / 2 - optionsWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - optionsWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 1){
			gameStartWhiteSprite.draw(screenWidth / 2 - gameStartWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - gameStartWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			timeAttackYellowSprite.draw(screenWidth / 2 - timeAttackYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - timeAttackYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			optionsWhiteSprite.draw(screenWidth / 2 - optionsWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - optionsWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
		if(menuSelection == 2){
			...
			timeAttackWhiteSprite.draw(screenWidth / 2 - timeAttackWhiteSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - timeAttackWhiteSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
			optionsYellowSprite.draw(screenWidth / 2 - optionsYellowSprite.getWidth() / 2 * Loader.scale + oldMovement * Loader.scale,
				screenHeight / 3 * 2 - optionsYellowSprite.getHeight() / 2 * Loader.scale, Loader.scale, Loader.scale, r);
		}
