...
public class Font{
	...
	public void addGlyph(int x, int y, int w, int h, char index){
		...
		glyphUVs[index] = new float[]{((x + w) * 1.0f) / (image.getWidth() * 1.0f), ((y + h) * 1.0f) / (image.getHeight() * 1.0f), (x * 1.0f) / (image.getWidth() * 1.0f),
			(y * 1.0f) / (image.getHeight() * 1.0f), ((x + w) * 1.0f) / (image.getWidth() * 1.0f), (y * 1.0f) / (image.getHeight() * 1.0f), (x * 1.0f) / (image.getWidth() * 1.0f),
			((y + h) * 1.0f) / (image.getHeight() * 1.0f)};
		height = max(h, height);
	}
