...
public class KeyListener{
	...
	public static boolean isKeyPressed(int key){
		if(glfwGetJoystickName(GLFW_JOYSTICK_1) != null){
			...
			try{
				if(key == GLFW_KEY_C && get().joystickButtonsPressed.get(Loader.joyA) == 1) return true;
			}
			catch(Exception ex){
				return get().keyPressed[key];
			}
			try{
				if(key == GLFW_KEY_X && get().joystickButtonsPressed.get(Loader.joyX) == 1) return true;
			}
			catch(Exception ex){
				return get().keyPressed[key];
			}
			try{
				if(key == GLFW_KEY_Z && get().joystickButtonsPressed.get(Loader.joyB) == 1) return true;
			}
			catch(Exception ex){
				return get().keyPressed[key];
			}
			try{
				if(key == GLFW_KEY_ESCAPE && get().joystickButtonsPressed.get(Loader.joyStart) == 1) return true;
			}
			catch(Exception ex){
				return get().keyPressed[key];
			}
			try{
				if(key == GLFW_KEY_ENTER && get().joystickButtonsPressed.get(Loader.joyBack) == 1) return true;
			}
			catch(Exception ex){
				return get().keyPressed[key];
			}
			if(key == GLFW_KEY_UP){
				try{
					...
					if(Loader.joyUpAxis == 1) if(get().joystickAxes.get(Loader.joyUp) > 0.1f) return true;
					if(Loader.joyUpAxis == -1) if(get().joystickAxes.get(Loader.joyUp) < -0.1f) return true;
				}
				catch(Exception ex){
					return (get().keyPressed[key]);
			...
			if(key == GLFW_KEY_DOWN){
				try{
					...
					if(Loader.joyDownAxis == 1) if(get().joystickAxes.get(Loader.joyDown) > 0.1f) return true;
					if(Loader.joyDownAxis == -1) if(get().joystickAxes.get(Loader.joyDown) < -0.1f) return true;
				}
				catch(Exception ex){
					return (get().keyPressed[key]);
			...
			if(key == GLFW_KEY_LEFT){
				try{
					...
					if(Loader.joyLeftAxis == 1) if(get().joystickAxes.get(Loader.joyLeft) > 0.1f) return true;
					if(Loader.joyLeftAxis == -1) if(get().joystickAxes.get(Loader.joyLeft) < -0.1f) return true;
				}
				catch(Exception ex){
					return get().keyPressed[key];
			...
			if(key == GLFW_KEY_RIGHT){
				try{
					...
					if(Loader.joyRightAxis == 1) if(get().joystickAxes.get(Loader.joyRight) > 0.1f) return true;
					if(Loader.joyRightAxis == -1) if(get().joystickAxes.get(Loader.joyRight) < -0.1f) return true;
				}
				catch(Exception ex){
					return get().keyPressed[key];
	...
	public static int getNextJoystickAxis(){
		for(int i = 0; i < get().joystickAxes.capacity(); i++) if(get().joystickAxes.get(i) > 0.1f || get().joystickAxes.get(i) < -0.1f) return i;
		return -1;
	}
	
	public static int getJoystickAxisSign(int axis){
		if(get().joystickAxes.get(axis) > 0.1f) return 1;
		if(get().joystickAxes.get(axis) < -0.1f) return -1;
		return 0;
	}
