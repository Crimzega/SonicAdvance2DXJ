...
public class Vector{
	...
	public void rotateAroundPoint(Vector origin, Vector angles){
		...
		y -= origin.y;
		double length = getLength();
		double xAngle = acos((x) / length) + angles.x, yAngle = asin((y) / length) + angles.y;
	...
	public void rotateAroundPointTo(Vector origin, Vector angles){
		...
		if(angles.getLength() != 0){
			double length = getLength(), currentAngle = getAngleOfVector(new Vector(x, y)), deltaAngle = getAngleOfVector(angles);
	...
	public void rotateAroundPointTo(Vector origin, double angles){
		x -= origin.x;
		y -= origin.y;
		double length = getLength(), currentAngle = getAngleOfVector(new Vector(x, y));
