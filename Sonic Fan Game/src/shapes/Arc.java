...
public class Arc extends Shape{
	...
	public Arc(Vector a, Vector center, double angle, Color color){
		this.color = color;
		Vector b = center, bot = a.subtract(b).normalize();
	...
	public Arc(Vector corner, double angle0, double angle1, double distance, Color color){
		this.color = color;
		Vector point0 = corner.add(new Vector(sin(angle0) * distance, cos(angle0) * distance)), point1 = corner.add(new Vector(-sin(angle1) * distance, -cos(angle1) * distance)),
			center = getLineLineIntersection(point0, new Vector(cos(angle0), sin(angle0)), point1, new Vector(cos(angle1), sin(angle1)));
	...
	public Vector[] getInverseShadow(Vector axis1){
	...
	public void draw(Graphics2D graphics, Vector offset){
		Vector bot = points[0].subtract(points[1]).normalize(), top = points[2].subtract(points[1]).normalize();
		double botAngle = acos(bot.x), topAngle = acos(top.x);
		if(bot.y > 0d) botAngle *= -1;
		if(top.y > 0d) topAngle *= -1;
		botAngle = limitAngle(botAngle);
		topAngle = limitAngle(topAngle);
